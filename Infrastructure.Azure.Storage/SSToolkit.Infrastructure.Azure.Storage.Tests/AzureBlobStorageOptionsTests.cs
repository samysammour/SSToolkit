namespace SSToolkit.Infrastructure.Azure.Storage.Tests
{
    ...
    {
        [Fact]
        public async Task GetContainerName_Tests()
        {
            var options = new AzureBlobStorageOptions();
            options.GetContainerName(null).ShouldBe("main");
            ...
            options.GetContainerName(null).ShouldBe("secondary");
            options.GetContainerName(string.Empty).ShouldBe("secondary");
            options.GetContainerName("different").ShouldBe("different");

            options.ContainerName = null;
            this.ExceptionAssertion<ArgumentNullException>(() => options.GetContainerName(null));
